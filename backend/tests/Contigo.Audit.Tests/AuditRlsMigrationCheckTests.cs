...
/// <summary>
/// The CI migration check task E01/F06/US02/T01 (story us-02-audit-baseline, AC-1) requires: it
/// runs as part of the normal `dotnet test` step in `.github/workflows/backend.yml`, migrates a
/// disposable Postgres instance, and fails the build if any tenant-scoped table in this module is
/// missing `ROW LEVEL SECURITY`, `FORCE ROW LEVEL SECURITY`, or an actual policy. Mirrors task
/// E01/F05/US01/T01's own check for Identity/Workspace
/// (`Contigo.Identity.Workspace.Tests.IdentityWorkspaceRlsMigrationCheckTests`) and task
/// E01/F04/US03/T01's for Documents/Contracts (`Contigo.Tenancy.Tests.TenantRlsMigrationCheckTests`).
///
...
/// working — and keeps rejecting an omission — as new tenant-scoped tables are added to this
/// module by future tasks, without anyone remembering to update this test.
/// </summary>
public sealed class AuditRlsMigrationCheckTests : IAsyncLifetime
{
    ...
    public async Task Every_tenant_scoped_table_has_forced_row_level_security_and_a_policy()
    {
        ...
        await using var db = new AuditDbContext(optionsBuilder.Options);
        ...
        try
        {
            foreach (var table in tenantScopedTables)
            {
                ...
                Assert.True(
                    rowSecurityEnabled,
                    $"[ADR-009/AC-1] Tenant-scoped table \"{table}\" does not have ROW LEVEL " +
                    "SECURITY enabled. Add a migration: ALTER TABLE ... ENABLE ROW LEVEL SECURITY.");
                Assert.True(
                    rowSecurityForced,
                    $"[ADR-009/AC-1] Tenant-scoped table \"{table}\" is not FORCE ROW LEVEL " +
                ...
                Assert.True(
                    policyCount >= 1,
                    $"[ADR-009/AC-1] Tenant-scoped table \"{table}\" has no RLS policy. Add a " +
    ...
    private static async Task<(bool Enabled, bool Forced)> ReadRowSecurityFlagsAsync(
        AuditDbContext db, string table)
    {
    ...
    private static async Task<long> ReadPolicyCountAsync(AuditDbContext db, string table)
    {
        var command = db.Database.GetDbConnection().CreateCommand();
        command.CommandText =
