...
/// <summary>
/// The CI migration check task E01/F05/US01/T01 (story us-01-workspace-roles, AC-1) requires: it
...
/// E01/F04/US03/T01's own check for Documents/Contracts
/// (`Contigo.Tenancy.Tests.TenantRlsMigrationCheckTests`).
///
...
/// </summary>
public sealed class IdentityWorkspaceRlsMigrationCheckTests : IAsyncLifetime
{
    ...
    public async Task Every_tenant_scoped_table_has_forced_row_level_security_and_a_policy()
    {
        ...
        await using var db = new IdentityWorkspaceDbContext(optionsBuilder.Options);
    ...
    private static async Task<(bool Enabled, bool Forced)> ReadRowSecurityFlagsAsync(
        IdentityWorkspaceDbContext db, string table)
    {
    ...
    private static async Task<long> ReadPolicyCountAsync(IdentityWorkspaceDbContext db, string table)
    {
        var command = db.Database.GetDbConnection().CreateCommand();
        command.CommandText =
