...
/// E03/F03/US01/T02, renewal-action) — mirrors <c>Contigo.Audit.Tests.AuditRlsMigrationCheckTests</c>
/// / <c>Contigo.Tenancy.Tests.TenantRlsMigrationCheckTests</c> exactly, scoped to this module's own
/// <see cref="RenewalsDbContext"/>. Runs as part of the normal `dotnet test` step in
...
/// </summary>
public sealed class RenewalActionRlsMigrationCheckTests : IAsyncLifetime
{
    ...
    public async Task Every_tenant_scoped_table_has_forced_row_level_security_and_a_policy()
    {
        ...
        RenewalsDbContextOptions.Configure(optionsBuilder, _postgres.GetConnectionString());

        await using var db = new RenewalsDbContext(optionsBuilder.Options);
    ...
    private static async Task<(bool Enabled, bool Forced)> ReadRowSecurityFlagsAsync(
        RenewalsDbContext db, string table)
    {
    ...
    private static async Task<long> ReadPolicyCountAsync(RenewalsDbContext db, string table)
    {
        var command = db.Database.GetDbConnection().CreateCommand();
        command.CommandText =
