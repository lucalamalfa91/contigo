...
/// <summary>
/// Single place that configures <see cref="AuditDbContext"/> provider options, so the runtime DI
/// path (<see cref="ServiceCollectionExtensions"/>), the design-time factory
/// (<see cref="AuditDbContextFactory"/>), and the test project can never drift apart on how the
...
public static class AuditDbContextOptions
{
        ...
        ITenantContext? tenantContext = null)
    {
        builder
            ...
            // Postgres/ADR-009 convention is snake_case (`tenant_id`, `audit_event`, ...);
